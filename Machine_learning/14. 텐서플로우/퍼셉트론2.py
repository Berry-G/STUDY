'''
    ...
    2. 역전파(BackPropagation)
        1) 가중치와 편향을 학습하기 위한 방법으로 고안 (1974, 1986)
            - 역전파가 XOR 문제 뿐만 아니라 더 복잡한 문제도 해결할 수 있음이 증명됨 ==> 인공신경망 재부흥
        2) 신경망의 오차(예측값과 실제값의 차이)를 출력층에서 입력층으로 거꾸로
            전파시켜 각 층(layer)의 가중치와 편향을 업데이트 함
...
# 퍼셉트론 학습 알고리즘 구현
def perceptron_fit(X, Y, epochs = 10):      # 10회 반복
    global W
    eta = 0.2
    ...
    for t in range(epochs):
        ...
        for i in range(len(X)):
            ...
            error = Y[i] - predict      # 오차 계산 (학습 10번하면서 오차 변화를 확인)
            W += eta * error * X[i]
